...
exports.TelecomAF = void 0;
var _reactNative = require("react-native");
var _NativeTelecomAntiFraud = _interopRequireDefault(require("./NativeTelecomAntiFraud"));
...
function getNative() {
  const mod = _NativeTelecomAntiFraud.default ?? _reactNative.NativeModules.RNTelecomAntiFraud;
  if (!mod) {
    throw new _errors.TelecomAFError('UNAVAILABLE', LINKING_ERROR);
  }
  return mod;
}
function requireConfig() {
  if (!config) {
    throw new _errors.TelecomAFError('NOT_CONFIGURED', 'TelecomAF.configure() must be called before using the SDK');
  }
  return config;
}
...
function configure(cfg) {
  if (!cfg?.host) {
    throw new _errors.TelecomAFError('NOT_CONFIGURED', 'TelecomAF.configure requires a non-empty host');
  }
  config = {
    host: cfg.host,
    sslPins: cfg.sslPins ?? [],
...
function isAvailable() {
  return (_NativeTelecomAntiFraud.default ?? _reactNative.NativeModules.RNTelecomAntiFraud) != null;
}
...
async function runInitialize() {
  ...
  for (let attempt = 1; attempt <= cfg.initMaxAttempts; attempt++) {
    try {
      await native.initialize(cfg.host);
      if (cfg.sslPins.length > 0) {
        try {
          await native.setSslPinning(cfg.sslPins);
        } catch (pinError) {
          console.log('[TelecomAF] setSslPinning failed:', pinError);
        }
      }
      return true;
    } catch (error) {
...
async function verifySmsCode(phone, code) {
  try {
    ...
    throw (0, _errors.toTelecomAFError)(error);
  }
}
...
async function makeOperationWithRecovery() {
  ...
  try {
    ...
    if (!(0, _errors.isRecoverableAccountError)(error)) {
      throw (0, _errors.toTelecomAFError)(error);
    }
    clearCache();
    await runInitialize();
    try {
      ...
      throw (0, _errors.toTelecomAFError)(retryError);
    }
  }
}
...
const TelecomAF = exports.TelecomAF = {
  configure,
  isAvailable,
  isConfigured,
  ensureInitialized,
