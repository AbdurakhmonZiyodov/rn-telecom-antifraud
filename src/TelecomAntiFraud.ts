...
const LINKING_ERROR =
  `The package 'rn-telecom-antifraud' doesn't seem to be linked. Make sure: \n\n` +
 ...
 */
function configure(cfg: TelecomAFConfig): void {
  if (!cfg?.host) {
 ...
 */
async function runInitialize(): Promise<boolean> {
...
async function verifySmsCode(phone: string, code: string): Promise<void> {
  try {
...
async function confirmFace(birthDate: string, document: string): Promise<void> {
  try {
...
async function makeOperation(): Promise<void> {
  try {
 ...
 */
async function makeOperationWithRecovery(): Promise<void> {
  const native = getNative();
  try {
...
async function detectFraud(code: string): Promise<void> {
  try {
...
async function setSslPinning(pins: string[]): Promise<void> {
  try {
...
/** Clear SSL pinning. */
async function clearSslPinning(): Promise<void> {
  try {
